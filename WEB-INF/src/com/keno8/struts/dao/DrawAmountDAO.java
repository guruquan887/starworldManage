...
public class DrawAmountDAO {
	...
	public void dealCancel(String express_ID,int userID) {  //用户取消
		...
		try {
			...
			ps = con.prepareStatement("update drawAmountApply set applytype=4,cancelType=1 where express_ID=?");
			ps.setString(1, express_ID);
			ps.execute();
	...
	public void updateDealDraw(String drawBeizhu,int userID,String express_ID) {
		Connection con = null;
		PreparedStatement ps = null;
		try {
/*		long amount = 0;
		...
		ps.execute();*/
		con = ds.getConnection();
		ps = con.prepareStatement("update drawAmountApply set applytype=1,cancelType=3,drawBeizhu=? where userID=? and express_ID=?");
