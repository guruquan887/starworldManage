...
public class GameShopCateTypeDAO {
	...
	public List<GameShopCateTypeDTO> GetRecordByPage(int pageindex,int pageSize,String where){
		...
		try {
			...
			while(rs.next()){
				...
				dto.setParentsID(rs.getInt("parentsID"));
				list.add(dto);
			}
	...
	public List<GameShopCateTypeDTO> select(int parentsID){
		List<GameShopCateTypeDTO> list = new ArrayList<GameShopCateTypeDTO>();
		Connection con = null;
		...
		try {
			if(parentsID==0){
				sql = "select * from ItemsType where levelID=1";
				sql = "select * from ItemsType";
			}
			else{
				sql = "select * from ItemsType where parentsID="+parentsID;
			}
			con = ds.getConnection();
	...
	public void updateTypeName(int typeID,String typeName) {
		Connection con = null;
		PreparedStatement ps = null;
		try {
			con =ds.getConnection();
			ps = con.prepareStatement("update ItemsType set typeName=? where typeID = ?");
