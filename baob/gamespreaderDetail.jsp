...
document.location.href="<%=request.getContextPath() %>/baob/gameUserBBTJ.do?action=SpreaderDeatailTCRecord&userID=<%=userID%>&termOne="+encodeURI(document.all.termOne.value)
...
function jumppage(value)
   {
       window.location.href="<%=request.getContextPath() %>/baob/gameUserBBTJ.do?action=SpreaderDeatailTCRecord&userID=<%=userID%>&curPage=" +value+"&pageSize=<%=pageSize %>&termOne=<%=termOne%>&selectOne=<%=selectOne %>&checkTime=<%=checkTime%>&startTime=<%=createTimeStart%>&endTime=<%=createTimeEnd%>&selectOrderBy=<%=selectOrderBy%>";
   }
 function changepage(pageNo){
	document.location.href="<%=request.getContextPath() %>/baob/gameUserBBTJ.do?action=SpreaderDeatailTCRecord&userID=<%=userID%>&curPage="+pageNo+"&termOne="+tone
  ...
  <table width="100%" border="0" cellpadding="2" cellspacing="1" class="table_margin">
    ...
    <c:forEach var="user" items="${userSpreaderRecord}">
      <tr onmouseover="this.className='trover'" onmouseout="this.className='trout'">
        ...
        <td class="tdcenter"><div align="center">${user.tax}%&nbsp;</div></td>
