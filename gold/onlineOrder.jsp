...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/gold/cardList.do?action=onlineOrder&termOne="+tone+"&selectOne=<%=selectOne %>&startTime=<%=startTime%>&endTime=<%=endTime%>&curPage=" +value;
   }
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/gold/cardList.do?action=onlineOrder&termOne="+tone+"&selectOne=<%=selectOne %>&startTime=<%=startTime%>&endTime=<%=endTime%>&curPage="+pageNo;
}
function delCheck(){
		var flag10=0;
...
function search(){
	document.location.href="<%=request.getContextPath()%>/gold/cardList.do?action=onlineOrder&termOne="+encodeURI(document.all.termOne.value)
...
<input name="endTime" type="text" onClick="WdatePicker()" value="" size="15" />
	    ...
	    <select name="selectOne" id="selectOne" class="input_width2">
		  ...
		  <option value="onlineID">订单号码</option>
        </select>
	...
	<form id="form1" name="form1" method="post" onsubmit="return delCheck()" action="<%=request.getContextPath()%>/gold/cardList.do?action=delete">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          ...
          <tr  onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter">${card.onlineID}</td>
            ...
            <td class="tdcenter">&nbsp;<c:if test="${card.orderStatus==0}"  var="true">未付款</c:if>
			<c:if test="${card.orderStatus==1}"  var="true">已付款待处理</c:if>
			<c:if test="${card.orderStatus==2}"  var="true">订单完成</c:if> </td>
          </tr>
		  </c:forEach>
          <tr>
