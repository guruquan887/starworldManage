...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/member/dhList.do?action=dhRecordConvertPresent&curPage=" +value+"&pageSize=<%=pageSize %>&termOne=<%=termOne%>&selectOne=<%=selectOne %>"+"&startTime="+document.all.startTime.value+"&endTime="+document.all.endTime.value;
   }

function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/member/dhList.do?action=dhRecordConvertPresent&curPage="+pageNo+"&termOne="+tone+"&selectOne=<%=selectOne %>&pageSize=<%=pageSize %>"+"&startTime="+document.all.startTime.value+"&endTime="+document.all.endTime.value;
}
function search(){
	document.location.href="<%=request.getContextPath()%>/member/dhList.do?action=dhRecordConvertPresent&termOne="+encodeURI(document.all.termOne.value)
	...
	<form id="form2" name="form2" method="post" onsubmit="return delCheck()" action="<%=request.getContextPath()%>/member/gamegoldList.do?action=delAll">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margingold">
          <tr>
            ...
            <td class="menutop" width="80">游戏</td>
            <td class="menutop" width="100">房间</td>
            <td class="menutop" width="100">兑换前钱庄银子</td>
            <td class="menutop" width="80">兑换前魅力点</td>
            <td class="menutop" width="100">兑换点数</td>
          ...
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            ...
            <td class="tdcenter">${user.curPresent}</td>
            <td class="tdcenter">${user.convertPresent}</td>
		   ...
		   <tr>
                <td class="tdcenter" colspan="11" align="center" valign="middle" bgcolor="#FFFFFF">
