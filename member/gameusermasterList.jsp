...
<%
	...
	//request.getSession().setAttribute("fromurl","gameusermasterList.do?"+request.getQueryString());
%>
...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/member/gamemasterList.do?action=gameMasterList&curPage=" +value+"&pageSize=<%=pageSize %>&orderby=<%=orderby%>&termOne=<%=termOne%>&selectOne=<%=selectOne %>";
   }
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/member/gamemasterList.do?action=gameMasterList&curPage="+pageNo+"&termOne="+tone
	+"&selectOne=<%=selectOne %>&pageSize=<%=pageSize %>"+
	"&orderby=<%=orderby %>";

}
function search(){
	document.location.href="<%=request.getContextPath()%>/member/gamemasterList.do?action=gameMasterList&termOne="+encodeURI(document.all.termOne.value)
	  ...
	  <form id="form1" name="form1" method="post" action="">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          ...
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            ...
            <td class="tdcenter_red"><span class="${user.bgcss}">${user.onLineName}</span></td>
            <td class="tdcenter"><a href="<%=request.getContextPath()%>/member/gamemasterList.do?action=zhFrozen&userState=${user.zhState}&userID=${user.userID}"><span class="${user.zhStateCss}">${user.zhStateName}</span></a> / <a href="<%=request.getContextPath()%>/member/gamemasterList.do?action=readright&uid=${user.userID}">权限</a> / <c:if test="${user.userRight==0}" var="true"><a href="<%=request.getContextPath()%>/member/gamemasterList.do?action=szbs&uid=${user.userID}">设为帐号</a></c:if>
	   <c:if test="${user.userRight>0}" var="true"><a href="<%=request.getContextPath()%>/member/gamemasterList.do?action=qxbs&uid=${user.userID}">取消帐号</a></c:if></td>
          </tr>
		  </c:forEach>
