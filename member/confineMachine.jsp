...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/member/ConfineList.do?action=confineMachine&termOne=<%=termOne%>&curPage=" +value;
   }
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/member/ConfineList.do?action=confineMachine&termOne=<%=termOne%>&curPage="+pageNo;
}
function search(){
	document.location.href="<%=request.getContextPath()%>/member/ConfineList.do?action=confineMachine&termOne="+encodeURI(document.all.termOne.value);
}
</script>
...
<body>
	...
	<input name="termOne" id="termOne" type="text" class="input_width3" />
	   <input name="Submit2" type="button" class="input" onClick="search()" value="查询" />
	    ...
	    <input name="Submit2" type="button" onclick="window.location.href='<%=request.getContextPath()%>/member/addconfineMachine.jsp'" class="input" value="新增" />
 </form>
	  
	</span>限制机器</Div>
	<form id="form2" name="form2" method="post" onsubmit="return delCheck()" action="<%=request.getContextPath()%>/member/ConfineList.do?action=delAllMachine">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          <tr>
		  <td width="30" class="menutop"><input type="checkbox" name="checkAll" value="checkAll" onClick="checkAllBox(0)" /></td>
            <td class="menutop">限制机器</td>
