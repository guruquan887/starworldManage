...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/member/gamegoldList.do?action=gamebankRecord&curPage=" +value+"&pageSize=<%=pageSize %>&termOne=<%=termOne%>&userID=<%=userID%>&selectOne=<%=selectOne %>"+"&startTime="+document.all.startTime.value+"&endTime="+document.all.endTime.value;
   }

function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/member/gamegoldList.do?action=gamebankRecord&curPage="+pageNo+"&termOne="+tone+"&selectOne=<%=selectOne %>&userID=<%=userID%>&pageSize=<%=pageSize %>"+"&startTime="+document.all.startTime.value+"&endTime="+document.all.endTime.value;
}
function search(){
	document.location.href="<%=request.getContextPath()%>/member/gamegoldList.do?action=gamebankRecord&userID=<%=userID%>&termOne="+encodeURI(document.all.termOne.value)
...
<body>
	<%--<div class="tab clearfix">
		<ul>
			<li class="active"><a href="<%=request.getContextPath()%>/member/gamegoldList.do?action=gamebankRecord&userID=<%=userID%>">钱庄记录</a></li>
...
<input name="endTime" type="text" onClick="WdatePicker()"  size="15" />
	...
	</span>钱庄记录(<a href="<%=request.getContextPath()%>/member/gamegoldList.do?action=gameproxybankRecord&type=1">代理记录</a>  <a href="<%=request.getContextPath()%>/member/gamegoldList.do?action=gamebankRecord&type=2">会员记录</a>)</Div>
	<form id="form2" name="form2" method="post" onsubmit="return delCheck()" action="<%=request.getContextPath()%>/member/gamegoldList.do?action=delAll">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margingold">
          <tr>
            ...
            <td class="menutop" width="150">汇款人</td>
            <td class="menutop" width="80">收款人</td>
            <td class="menutop" width="100">交易类型</td>
            <%--<td class="menutop" width="100">汇款人现金</td>
            <td class="menutop" width="150">汇款人钱庄</td>
            <td class="menutop" width="80">收款人现金</td>
            <td class="menutop" width="100">收款人钱庄</td>--%>
          ...
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            ...
            <td class="tdcenter">&nbsp;<a href="<%=request.getContextPath()%>/member/Recordlist.jsp?userID=${user.sourceUserID}">${user.sourceAccounts}</a></td>
            <td class="tdcenter">&nbsp;<a href="<%=request.getContextPath()%>/member/Recordlist.jsp?userID=${user.targetUserID}">${user.targetAccounts}</a></td>
