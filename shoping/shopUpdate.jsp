...
<body>
	<Div class="title">商品修改</Div>
		<form id="form1" name="form1" method="post" onSubmit="return gameshopaddcheck()" action="<%=request.getContextPath()%>/shoping/gameshopList.do?action=updateMall">
		<input name="id" type="hidden" value="${gameshop.id}" />
         ...
         <c:forEach var="gsType" items="${typelist}">
           <option value="${gsType.typeID}" <c:if test="${gsType.typeID==gameshop.typeID}" var="true"> selected="selected"</c:if>>${gsType.typeName}</option>
         </c:forEach>
       </select></td>
            </tr>
            <tr>
              <td class="tdright_new">商品名称：</td>
              <td class="tdlefts"><input type="text" name="mallName" id="mallName" value="${gameshop.mallName}"/></td>
            </tr>
            <tr>
              <td class="tdright_new">普通价格：</td>
              <td class="tdlefts"><input type="text" name="price_gold" id="price_gold" onkeyup="isNumber(this)" value="${gameshop.price_gold}"/></td>
            </tr>
            <tr>
              <td class="tdright_new">会员价格：</td>
              <td class="tdlefts"><input type="text" name="vipPrice" id="vipPrice" onkeyup="isNumber(this)" value="${gameshop.vipPrice}"/></td>
            </tr>
            <tr>
              <td class="tdright_new">商品数量：</td>
              <td class="tdlefts"><input type="text" name="count" id="count" onkeyup="isNumber(this)" value="${gameshop.count}"/></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td class="tdlefts"><textarea name="cateNote">${gameshop.descript}</textarea>
