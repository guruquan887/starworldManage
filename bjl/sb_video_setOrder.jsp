...
<script type="text/javascript"><!--
   ...
   {
      window.location.href="<%=request.getContextPath() %>/bjl/sbgameResultManage.do?pageIndex="+value+"&action=preSetOrder&select_resultOfWinLost=<%=select_resultOfWinLost %>&hgName=tone";
   }
   
   function search(){
    //var hg_Name = document.getElementById("_hgName").value;
    var str ="<%=request.getContextPath() %>/bjl/sbgameResultManage.do?action=preSetOrder&beforeXh="+document.getElementById("beforeXh").value+"&select_resultOfWinLost="+document.all.select_resultOfWinLost.options[document.all.select_resultOfWinLost.selectedIndex].value+"&hgName="+encodeURI(document.getElementById("_hgName").value);
...
<body>
<form id="form1" name="form1" method="post" action="<%=request.getContextPath()%>/bjl/sbgameResultManage.do?action=setOrder">
...
<input type="hidden" name="roomID" value="<%=roomID %>" id="roomID" />
  <table width="100%" border="0" cellpadding="2" cellspacing="1" class="table_margin">
    <tr align="center">
      <td height="25" colspan=6 class="menutop"><div style="width:90%; float:left; text-align:center; line-height:25px;"><a href="<%=request.getContextPath() %>/bjl/video.do?action=list"><strong>替播|视频列表</strong></a></div>
      </td>
    </tr>
      <tr>
    <td class="keno_td_left_menu">&nbsp;荷官名称<input type="text"  name="_hgName" id="_hgName" value="<%=hgName %>" size="15" />&nbsp;显示&nbsp;<select name="select_resultOfWinLost" size="1" id="select_resultOfWinLost"> 
		<option value="DAN">单</option>
		<option value="SHUANG">双</option>
  ...
  <table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
    <tr class="title">
      ...
      <td align="center" class="menutop"><b>视频结果</b></td>
     ...
     <c:forEach var="dto" items="${videoList}">
		<tr onMouseOver="javascript:this.style.backgroundColor='#BFDFFF'" onMouseOut="javascript:this.style.backgroundColor=''" style="font-family:宋体;">
	      ...
	      <td height="25" align="center" class="tdcenter">${dto.resultOfWinLost }</td>
		  <td height="25" align="center" class="tdcenter">${dto.hgName }</td>
        </tr>
	</c:forEach>
	<c:if test="${returnInfo != null}" var='true'>
    <tr align="center"><td colspan="8" class="kai_keno_title"><font color="red"><c:out value="${returnInfo}"></c:out></font></td></tr>
    </c:if>
    <c:if test="${page.totalPage>0}" var='true'>
	<tr>
      <td colspan="8" class="tdlefts"><label> &nbsp;&nbsp;&nbsp;
    ...
    <td colspan="8">
			...
			<c:if test="${page.totalPage>0}" var='true'>
		　<a href="<%=request.getContextPath() %>/bjl/sbgameResultManage.do?action=preSetOrder&pageIndex=1&select_resultOfWinLost=<%=select_resultOfWinLost %>&hgName=<%=hgName %>">首页</a> 
			<a href="<%=request.getContextPath() %>/bjl/sbgameResultManage.do?action=preSetOrder&pageIndex=${page.curPage-1}&select_resultOfWinLost=<%=select_resultOfWinLost %>&hgName=<%=hgName %>">上页</a> 
			<select name="select" onChange="jumppage(this.value);">
				...
				</select>&nbsp;&nbsp;<a href="<%=request.getContextPath() %>/bjl/sbgameResultManage.do?action=preSetOrder&pageIndex=${page.curPage+1 }&select_resultOfWinLost=<%=select_resultOfWinLost %>&hgName=<%=hgName %>">下页</a> 
				<a href="<%=request.getContextPath() %>/bjl/sbgameResultManage.do?action=preSetOrder&pageIndex=${page.totalPage}&select_resultOfWinLost=<%=select_resultOfWinLost %>&hgName=<%=hgName %>">末页</a>				</c:if>				</td></tr></table>   </td>
   </tr>
  </c:if>
  </table>
