...
<script type="text/javascript"><!--
   ...
   {
      window.location.href="<%=request.getContextPath() %>/bjl/sbvideo.do?pageIndex="+value+"&action=list";
   }
   
   function deleteObj(kid)
   {
    if(confirm("确定要删除该信息吗？")){
    window.location.href="<%=request.getContextPath() %>/bjl/sbvideo.do?id="+kid+"&action=deleteVideo";
	}
   }  
   function preUpdateObj(id){
     window.location.href="<%=request.getContextPath() %>/bjl/sbvideo.do?id="+id+"&pageIndex=<%=pageIndex %>&action=preUpdateVideo&pageSize=<%=pageSize %>";
   }
--></script>
<body>
<form id="form1" name="form1" method="post" action="adminDel.do">
  ...
  <table width="100%" border="0" cellpadding="2" cellspacing="1" class="table_margin">
    <tr class="title">
      ...
      <td align="center" class="menutop"><b>开奖结果</b></td>
      <td align="center" class="menutop"><b>荷官</b></td>
      <td align="center" class="menutop"><b>视频长度（秒）</b></td>
      <td align="center" class="menutop"><b>开始投注时间（秒）</b></td>
      <td align="center" class="menutop"><b>结束投注时间（秒）</b></td>
      <%--<td align="center" class="td1"><b>视频类型</b></td>--%>
    ...
    <c:forEach var="dto" items="${videoList}">
		<tr>
	      ...
	      <td height="25" align="center" class="tdcenter">${dto.videoID }</td>
	      ...
	      <td height="25" align="center" class="tdcenter">${dto.startTime}</td>
	      <td height="25" align="center" class="tdcenter">${dto.endTime}</td>
    ...
    <td colspan="11">
		...
		　<a href="<%=request.getContextPath()%>/bjl/sbvideo.do?action=list&pageIndex=1">首页</a> 
			<a href="<%=request.getContextPath()%>/bjl/sbvideo.do?action=list&pageIndex=${page.curPage-1}">上页</a> 
			<select name="select" onChange="jumppage(this.value);">
				...
				</select>&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/bjl/sbvideo.do?action=list&pageIndex=${page.curPage+1 }">下页</a> 
				<a href="<%=request.getContextPath()%>/bjl/sbvideo.do?action=list&pageIndex=${page.totalPage}">末页</a>				</c:if>				</td></tr></table>   </td>
   </tr>
  </table>
