...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/web/GameMatchInfoList.do?action=gameMatchList&curPage="+value+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>&termOne="+tone;
   }
   
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/web/GameMatchInfoList.do?action=gameMatchList&curPage="+pageNo+"&termOne="+tone+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>";
}
function undoCheck(){
	var flag10=0;
	   ...
	   <tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          <tr>
            ...
            <td width="60" class="menutop">比赛标识</td>
            <td class="menutop">比赛名称</td>
            <td class="menutop">比赛时间</td>
            <td class="menutop">赛事状态</td>
            <td class="menutop">冻结状态</td>
            <td class="menutop">新增日期</td>
          </tr>
		  <c:forEach var="issue" items="${gameMatchlist}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><label>
              <input type="checkbox" name="checkbox" value="${issue.matchID}" />
            </label></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?matchID=${issue.matchID}'" class="tdcenter">${issue.matchID}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?matchID=${issue.matchID}'" class="tdcenter">${issue.matchTitle}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?matchID=${issue.matchID}'" class="tdcenter">${issue.applyBeginDate}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?matchID=${issue.matchID}'" class="tdcenter"><c:if test="${issue.matchStatus==0}" var="true">未开始</c:if><c:if test="${issue.matchStatus==1}" var="true">进行中</c:if><c:if test="${issue.matchStatus==2}" var="true">已结束</c:if></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?matchID=${issue.matchID}'" class="tdcenter"><c:if test="${issue.nullity==0}" var="true">否</c:if><c:if test="${issue.nullity==1}" var="true">是</c:if></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?matchID=${issue.matchID}'" class="tdcenter">${issue.collectDate}</td>
          </tr>
		  </c:forEach>
          <tr>
		  <tr>
