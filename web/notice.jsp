...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/web/GameMatchInfoList.do?action=noticeList&curPage="+value+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>&termOne="+tone;
   }
   
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/web/GameMatchInfoList.do?action=noticeList&curPage="+pageNo+"&termOne="+tone+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>";
}
function undoCheck(){
	var flag10=0;
	   ...
	   <tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          <tr>
            ...
            <td width="60" class="menutop">标题</td>
            <td class="menutop">窗口大小</td>
            <td class="menutop">开始时间</td>
            <td class="menutop">结束时间</td>
            <td class="menutop">禁用状态</td>
          </tr>
		  <c:forEach var="issue" items="${noticeList}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><label>
              <input type="checkbox" name="checkbox" value="${issue.noticeID}" />
            </label></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?noticeID=${issue.noticeID}'" class="tdcenter">${issue.subject}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?noticeID=${issue.noticeID}'" class="tdcenter">${issue.width}X${issue.height}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?noticeID=${issue.noticeID}'" class="tdcenter">${issue.startDate}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?noticeID=${issue.noticeID}'" class="tdcenter">${issue.overDate}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?noticeID=${issue.noticeID}'" class="tdcenter"><c:if test="${issue.nullity==0}" var="true">否</c:if><c:if test="${issue.nullity==1}" var="true">是</c:if></td>
            </tr>
		  </c:forEach>
          <tr>
