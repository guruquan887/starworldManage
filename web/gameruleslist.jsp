...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/web/gamerulesList.do?curPage="+value+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>&termOne="+tone;
   }
   
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/web/gamerulesList.do?curPage="+pageNo+"&termOne="+tone+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>";
}
function undoCheck(){
	var flag10=0;
	  ...
	  规则管理</Div>
	   ...
	   <tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
		  ...
		  <c:forEach var="issue" items="${gameRuleslist}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><label>
              <input type="checkbox" name="checkbox" value="${issue.kindID}" />
            </label></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?kindID=${issue.kindID}'" class="tdcenter">${issue.kindID}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?kindID=${issue.kindID}'" class="tdcenter">${issue.kindName}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?kindID=${issue.kindID}'" class="tdcenter"><c:if test="${issue.nullity==0}" var="true">否</c:if><c:if test="${issue.nullity!=0}" var="true"><font color="#FF0000">是</font></c:if></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?kindID=${issue.kindID}'" class="tdcenter">${issue.collectDate}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?kindID=${issue.kindID}'" class="tdcenter">${issue.modifyDate}</td>
          </tr>
		  </c:forEach>
          <tr>
		  <tr>
