...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/web/issueList.do?curPage="+value+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>&termOne="+tone;
   }
   
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/web/issueList.do?curPage="+pageNo+"&termOne="+tone+"&nullity=<%=nullity %>&pageSize=<%=pageSize %>";
}
function undoCheck(){
	var flag10=0;
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          ...
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><label>
              <input type="checkbox" name="checkbox" value="${issue.issueID}" />
            </label></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?issueID=${issue.issueID}'" class="tdcenter">${issue.issueID}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?issueID=${issue.issueID}'" class="tdcenter">${issue.issueTitle}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?issueID=${issue.issueID}'" class="tdcenter"><c:if test="${issue.nullity==0}" var="true">否</c:if><c:if test="${issue.nullity!=0}" var="true"><font color="#FF0000">是</font></c:if></td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?issueID=${issue.issueID}'" class="tdcenter">${issue.collectDate}</td>
            <td onclick="window.location.href='<%=request.getContextPath()%>/web/issuePreUpdate.do?issueID=${issue.issueID}'" class="tdcenter">${issue.modifyDate}</td>
          </tr>
		  </c:forEach>
          <tr>
		  <tr>
