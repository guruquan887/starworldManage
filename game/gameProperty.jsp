...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=gameProperty&curPage=" +value+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
   }
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=gameProperty&curPage="+pageNo+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
}
function search(){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=gameProperty&termOne="+encodeURI(document.all.termOne.value)
...
<body>
	<Div class="title"><span><form id="form1" name="form1" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
      ...
      <input name="Submit" type="button" onClick="window.location.href='<%=request.getContextPath()%>/game/createMachine.jsp'" class="input" value="禁用" /><input name="Submit" type="button" onClick="window.location.href='<%=request.getContextPath()%>/game/createMachine.jsp'" class="input" value="启用" /></form>
	</span> 道具管理</Div>
	<form id="form2" name="form2" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          <tr>
            ...
            <td class="menutop"> 现金价格 </td>
            <td class="menutop"> 银子价格 </td>
            <td class="menutop"> 折扣价 </td>
            <td class="menutop"> 发行范围 </td>
            <td class="menutop"> 使用范围 </td>
            <td class="menutop"> 赠送魅力 </td>
            <td class="menutop"> 接受魅力 </td>
            <td class="menutop"> 状态 </td>
          </tr>
		  <c:forEach var="playPresent" items="${gamePropertylist}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><input type="checkbox" id="checkbox" value="${playPresent.serverID}" name="checkbox" /></td>
            <td class="tdcenter"></td>
            <td class="tdcenter">${playPresent.name}</td>
            <td class="tdcenter">${playPresent.cash}&nbsp;</td>
            <td class="tdcenter">${playPresent.gold}</td>
            <td class="tdcenter">${playPresent.disCount}</td>
            <td class="tdcenter">${playPresent.issueArea}</td>
            <td class="tdcenter">${playPresent.serviceArea}</td>
            <td class="tdcenter">${playPresent.sendLoveLiness}</td>
            <td class="tdcenter">${playPresent.recvLoveLiness}&nbsp;</td>
