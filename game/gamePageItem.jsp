...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=gamePageItem&curPage=" +value+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
   }
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=gamePageItem&curPage="+pageNo+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
}
function search(){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=gamePageItem&termOne="+encodeURI(document.all.termOne.value)
...
<body>
	...
	<form id="form2" name="form2" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          <tr>
            <td colspan="11" class="title"><div align="left"><a href="<%=request.getContextPath()%>/game/gameSystem.do?action=gameGameItem">模块</a> <a href="<%=request.getContextPath()%>/game/gameSystem.do?action=gameTypeItem">类型</a> <a href="<%=request.getContextPath()%>/game/gameSystem.do?action=gameKindItem">游戏</a> <a href="<%=request.getContextPath()%>/game/gameSystem.do?action=gameNodeItem">节点</a> <a href="<%=request.getContextPath()%>/game/gameSystem.do?action=gamePageItem">自定义页</a></div></td>
          </tr>
          <tr>
            <td  class="menutop"><input type="checkbox" name="checkAll" value="checkAll" onClick="checkAllBox(0)" /></td>
            <td class="menutop">页面标识</td>
            <td class="menutop">页面名称</td>
            <td class="menutop">游戏名称</td>
            <td class="menutop">节点名称</td>
            <td class="menutop">操作类型</td>
            <td class="menutop">排序</td>
            <td class="menutop">资源路径</td>
            <td class="menutop">禁用状态</td>
            <td class="menutop">管理</td>
          </tr>
		  <c:forEach var="machine" items="${gamePagelist}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><input type="checkbox" id="checkbox" value="${machine.gameID}" name="checkbox" /></td>
            <td class="tdcenter">${machine.pageID}</td>
            <td class="tdcenter">${machine.displayName}</td>
            <td class="tdcenter">${machine.kindName}&nbsp;</td>
            <td class="tdcenter">${machine.nodeName}&nbsp;</td>
            <td class="tdcenter">操作类型${machine.operateType}&nbsp;</td>
            <td class="tdcenter">${machine.sortID}&nbsp;</td>
            <td class="tdcenter">${machine.responseUrl}&nbsp;</td>
