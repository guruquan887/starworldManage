...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=SystemMessage&curPage=" +value+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
   }
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=SystemMessage&curPage="+pageNo+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
}
function search(){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=SystemMessage&termOne="+encodeURI(document.all.termOne.value)
...
<body>
	<Div class="title"><span><form id="form1" name="form1" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
	  ...
	  <input name="Submit" type="button" onClick="window.location.href='<%=request.getContextPath()%>/game/createMachine.jsp'" class="input" value="新建" /><input name="Submit" type="button" onClick="window.location.href='<%=request.getContextPath()%>/game/createMachine.jsp'" class="input" value="冻结" /><input name="Submit" type="button" onClick="window.location.href='<%=request.getContextPath()%>/game/createMachine.jsp'" class="input" value="解冻" /></form>
	</span> 系统消息</Div>
	<form id="form2" name="form2" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          <tr>
            ...
            <td class="menutop"> 消息范围</td>
            <td class="menutop"> 消息内容</td>
            <td class="menutop"> 频率（秒）</td>
            <td class="menutop"> 禁止标志</td>
          </tr>
		  <c:forEach var="playPresent" items="${SystemMessagelist}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><input type="checkbox" id="checkbox" value="${playPresent.serverID}" name="checkbox" /></td>
            <td class="tdcenter"></td>
            <td class="tdcenter">${playPresent.startTime}</td>
            <td class="tdcenter">${playPresent.concludeTime}&nbsp;</td>
            <td class="tdcenter">${playPresent.serverRange}</td>
            <td class="tdcenter">${playPresent.messageString}</td>
            <td class="tdcenter">${playPresent.timeRate}</td>
