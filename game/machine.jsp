...
<body>
	<Div class="title"><span><form id="form1" name="form1" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
	    ...
	    <select name="selectOne" class="input_width1" id="selectOne">
	      <option value="machine">机器名称</option>
        </select>	  
	...
	<form id="form2" name="form2" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
          <tr>
            ...
            <td class="menutop">服务器名</td>
            <td class="menutop">地址</td>
            <td class="menutop">端口</td>
            <td class="menutop">机器码</td>
          </tr>
		  <c:forEach var="machine" items="${machinelist}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            <td class="tdcenter"><input type="checkbox" id="checkbox" value="${machine.dbInfoID}" name="checkbox" /></td>
            <td class="tdcenter">${machine.information}</td>
            <td class="tdcenter">${machine.dbAddr}</td>
            <td class="tdcenter">${machine.dbPort}&nbsp;</td>
            <td class="tdcenter">${machine.machineID}&nbsp;</td>
          </tr>
          </c:forEach>
		   <c:if test="${returnInfo!=''}"  var="true">
		  <tr>
                <td class="tdcenter" colspan="15" align="center" valign="middle" bgcolor="#FFFFFF">
