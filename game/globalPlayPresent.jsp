...
function jumppage(value)
   {
      window.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=machine&curPage=" +value+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
   }
function changepage(pageNo){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=machine&curPage="+pageNo+"&termOne="+encodeURI(document.all.termOne.value)+"&selectOne="+document.all.selectOne.options[document.all.selectOne.selectedIndex].value;
}
function search(){
	document.location.href="<%=request.getContextPath()%>/game/gameSystem.do?action=machine&termOne="+encodeURI(document.all.termOne.value)
...
<body>
	<Div class="title"><span><form id="form1" name="form1" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
	    <input name="Submit" type="button" class="input" value="封顶设置" onclick="tjSubmit(0)"/>
	...
	<form id="form2" name="form2" method="post" action="<%=request.getContextPath()%>/game/RoomLine.do?action=list">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_margin">
		  ...
		  <c:forEach var="playPresent" items="${globalPlayPresentlist}">
          <tr onmouseOver="this.className='trover'" onmouseOut="this.className='trout'">
            ...
            <td class="tdcenter">${playPresent.presentMember}</td>
            <td class="tdcenter">${playPresent.cellPlayPresnet}&nbsp;</td>
            <td class="tdcenter">${playPresent.cellPlayTime}</td>
            <td class="tdcenter">${playPresent.startPlayTime}</td>
            <td class="tdcenter">${playPresent.cellOnlinePresent}</td>
            <td class="tdcenter">${playPresent.cellOnlineTime}</td>
            <td class="tdcenter">${playPresent.startOnlineTime}</td>
            <td class="tdcenter">${playPresent.isPlayPresentName}</td>
            <td class="tdcenter">${playPresent.isOnlinePresentName}</td>
            <td class="tdcenter">${machine.collectDate}&nbsp;</td>
          </tr>
          </c:forEach>
		   <c:if test="${returnInfo!=''}"  var="true">
		  <tr>
                <td class="tdcenter" colspan="22" align="center" valign="middle" bgcolor="#FFFFFF">
				<font color="red"><c:out value="${returnInfo}"></c:out></font></td>
       </tr>
	   </c:if>
          <tr>
            <td colspan="12" class="tdright_new">总记录:${page.totalRecord}条/${page.totalPage}页
